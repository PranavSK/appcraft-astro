...
const DropdownMenuShortcut: Component<ComponentProps<'span'>> = (props) => {
  ...
  return (
    <span
      class={cx('ml-auto text-xs tracking-widest opacity-60', props.class)}
      {...rest}
    />
  )
}
...
> = (props) => {
  ...
  return (
    <DropdownMenuPrimitive.Separator
      class={cx('-mx-1 my-1 h-px bg-muted', props.class)}
      {...rest}
    />
  )
}
...
> = (props) => {
  ...
  return (
    <DropdownMenuPrimitive.GroupLabel
      class={cx('px-2 py-1.5 text-sm font-semibold', props.class)}
      {...rest}
    />
  )
}
